...
from src.model import create_model, create_loss, create_metric, create_optimizer
from src.model.MyBatchNorm2d import MyBatchNorm2d
...
class ATPClient(BaseClient):
    ...
    def ppld_batch_compute(self,model,adapt_les,args,dataset='test'):
        total_plpd = 0
        patch_len = 2
        ...
        for *X, Y in dataloader:
            ...
            with torch.no_grad():
                ...
                x_prime = torch.cat([x.to(self.device) for x in X], dim=0)
                ...
                # Accumulate PLPD values for all samples in the batch
                total_plpd += plpd.sum().item()  # Add the sum of PLPD for this batch
                num_examples = len(X[0])
                total_examples += num_examples
        avg_plpd = total_plpd / total_examples
        return avg_plpd
