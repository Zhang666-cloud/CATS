...
class TTPBaseServer(BaseServer):
    ...
    def run(self, args):
        ...
        for rnd in range(1, self.gm_rounds + 1):
            ...
            self.adapt_and_eval(args)
        avg_client_plpd=self.plpd_compute(args)
        print(avg_client_plpd)

    def plpd_compute(self,args):
        ...
        selected_cids = [self.train_idx2cid[idx] for idx in range(self.num_train_clients)]
        plpds = []
        for cid in tqdm(selected_cids):
            client = self.train_clients[cid]
            client_plpds=client.ppld_batch_compute(self.model, self.adapt_lrs, args, 'test')
            plpds.append(client_plpds)
        plpd_global_avg = sum(plpds) / len(plpds)
        return plpd_global_avg
