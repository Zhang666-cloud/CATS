...
from dataset.Office_home_Aug import Office_home_Aug
from utils import pickle_save, pickle_load
from partition.create_partition import create_partition_office_home
...
def main(args):
    ...
    if not os.path.exists(args.domain_path):
        datasets = Office_home_Aug(root=args.data_dir, test_idx=args.partition_seed)
    ...
    for environment in range(4):
        ...
        # 设置partition_seed来改变目标客户端的测试数据集（Office_home）
        # 目标客户端：0为Art，1为Clipart，2为Product, 3为Real_World
        ...
        train_partitions, test_partitions, partition_idxs = create_partition_office_home([dataset, ], args, environment)
